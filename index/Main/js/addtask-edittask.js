...
function checkUser(AssignedUsers, JoinUsers) {
  for (let Worker of AssignedUsers) {
    let usersFound = JoinUsers.find(
      (benutzer) => benutzer.email === Worker.email,
    );
    if (usersFound) {
...
function assignedCheck(x) {
  document
    .getElementById(`tinyAccountCardCheckedNone${x}`)
...
function assignedCheckNone(x) {
  ...
  document
    .getElementById(`tinyAccountCardChecked${x}`)
    .classList.remove("d-none");
...
function editSubtask(m) {
  ...
  editableTask.addEventListener("keydown", (event) => {
    if (event.key === "Enter" || event.keyCode === 13) {
      if (editableTask.value != "") subtaskTemp[m] = editableTask.textContent;
